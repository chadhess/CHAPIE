"""
examples/cargo_ectol_h2_350bar_mission.py — Generic cargo eCTOL H2 hybrid mission (350 bar).

Illustrative example using representative performance values for a small
H2 hybrid cargo eCTOL aircraft with 350 bar hydrogen storage. Intended to
demonstrate CHAPIE usage.

Run directly:
    python cargo_ectol_h2_350bar_mission.py
"""
import sys
import os
...
def run_single(payload_kg: float = 200.0, target_range_nm: float = None) -> None:
    ...
    """
    ac      = CargoECTOL()
    fuel_kg = h2_350bar.fuel_mass_kg(BAY_VOL_M3, N_BAYS)
    ...
    print(f"\nCargo eCTOL  |  350 bar H2  |  Payload: {payload_kg:.1f} kg  "
...
def range_payload_sweep() -> None:
    ...
    print(f"\nRange-Payload trade  |  Cargo eCTOL 350 bar H2  |  H2 fuel: {fuel_kg:.1f} kg")
    print(f"  {'Payload [lbs]':>15}  {'Range [nm]':>12}  {'TOGW [lbs]':>12}  {'MTOW':>6}")
    print(f"  {'-'*51}")
